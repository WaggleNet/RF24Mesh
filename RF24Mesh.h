...
public:
  ...
  */
   ...
   */
  int16_t getNodeID(address_t Address=MESH_BLANK_ID);
  ...
  */
   ...
   */
  int16_t getAddress(nodeid_t nodeID);

  /**
   * Write to a specific node by RF24Network address.
   *
   */
  bool write(nodeid_t to_node, const void* data, uint8_t msg_type, size_t size );
...
#if !defined RF24TINY
  typedef struct{
	nodeid_t nodeID;       /**< NodeIDs and addresses are stored in the addrList array using this structure */
	address_t Address;  /**< NodeIDs and addresses are stored in the addrList array using this structure */
  }addrListStruct;
